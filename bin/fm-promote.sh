...
# that delivers it to the current worker. That hand-off line names the task id,
# never the fm-<task-id> window label or the flattened fm/<task-id> branch name,
# so the operator can run it as printed instead of repairing an identifier by
# hand while the promoted worker waits for its contract (regression coverage
# lives in tests/fm-task-delivery.test.sh). Those instructions carry the
# scratch-state inventory, the clean default-branch base, the fm/<task-id>
# branch, and - rendered from bin/fm-dod-lib.sh, the single owner an ordinary
# ship brief also uses - the mode-specific Definition of done, so a promoted
...
# instructions also carry `# Task` with `## Captain's intent` preserved from the
# scout brief and promotion's ship-time instructions under `## Firstmate spec`;
# the scout-time spec remains context but is not relabeled as the ship spec.
