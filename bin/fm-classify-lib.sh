...
# 6: two independent version-5 readings exist in the wild - one that fixed the
# unbracketed-correlation-token parsing above, and one that fixed the mid-line
# cursor commit described in status_open_decisions_incremental's header. A
# cursor persisted under either one predates the other, so version 5 cannot
# identify a cursor folded under both and both readings must be rebuilt from
# byte 0.
