...
# same when a task kind changes, because kind changes the fold below. It is
# bumped for a fixed fold BUG too, for the same reason: a cursor written by the
# broken version can already carry a wrong open set, and only a rebuild from
# byte 0 retires it.
#
...
# discarded.
# 10: the incremental fold no longer commits its cursor mid-line (see the
...
status_open_decisions_incremental() {  # <status-file> [<captured-end-offset>]
  ...
  local target_cursor complete_size partial_file emit='' kind fold_version
  [ -f "$f" ] && [ -r "$f" ] && [ ! -L "$f" ] || return 0
  kind=$(_fm_status_kind "$f")
  ...
  if [ "$offset" -lt "$size" ]; then
    ...
    if [ "$complete_size" -lt "$chunk_size" ]; then
      ...
      while IFS= read -r line || [ -n "$line" ]; do
        emit=$(_fm_decision_fold_line "$emit" "$line" "$resolve" "$held" "$kind")
      done < "$partial_file"
      rm -f "$partial_file"
    fi
    rm -f "$chunk_file"
...
status_presentation_cursor_offset() {  # <status-file>
  local f=$1 state task manifest data row_task offset ident backstop extra cur_ident size legacy probe
...
status_commit_presentation_snapshot() {  # <state> <snapshot>
  ...
  while IFS=$(printf '\t') read -r task endpoint ident; do
    ...
    # The previously committed offset is this row's third field; it is itself a
    # committed boundary, so the scan only ever covers the new span.
    prev=0
    case "$rows" in
      *$'\n'"$task"$'\t'"$ident"$'\t'*)
        prev=${rows#*$'\n'"$task"$'\t'"$ident"$'\t'}
    ...
    endpoint=$(_fm_status_line_boundary_end "$f" "$prev" "$endpoint") \
      || { rm -f "$tmp"; return 1; }
    # The outcome backstop is a dedupe receipt compared against endpoints, never
    # a byte position anything reads lines from, so it is recorded as captured.
