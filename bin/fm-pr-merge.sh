...
# Only a merge the forge confirmed landed moves the card: every refused, failed,
# or still-queued path above has already exited. A task with no board link
# reaches no board at all, and a board write that does not land leaves the card
# stale for the next poll rather than changing the merge's own outcome.
# bin/fm-board.sh owns the mechanics.
