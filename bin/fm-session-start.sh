...
for meta in "$STATE"/*.meta; do
  ...
  if [ -n "$window" ]; then
    ...
    elif [ "$(fm_meta_get "$meta" endpoint_closed)" = "${target:-$window}" ]; then
      # bin/fm-control.sh stand-down closed it on purpose; nothing to recover.
      printf 'endpoint: closed at stand-down (backend=%s window=%s)\n' "$backend" "$window"
    else
