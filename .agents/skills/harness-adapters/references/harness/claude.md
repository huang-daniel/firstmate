...
The bypass-permissions confirmation is a third, separate dialog, scoped to the active Claude config directory rather than the path, and pre-registration does not address it: Claude records the answer as `skipDangerousModePermissionPrompt` in that directory's `settings.json` (`${CLAUDE_CONFIG_DIR:-$HOME/.claude}/settings.json`), and `../../../bin/fm-spawn.sh` forwards a set `CLAUDE_CONFIG_DIR` onto every claude launch, so a config directory the operator has accepted the dialog in stays accepted for workers that inherit it.
...
Firstmate cannot move a selection with Enter, Escape, and C-c alone, so it cannot accept this dialog at all, and an operator accepts it once per config directory instead.
