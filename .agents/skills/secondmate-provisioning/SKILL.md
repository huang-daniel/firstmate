...
A successful update restarts every stale live mate of both placements on its own, including one whose home was already on the target commit, unless it is provably busy; the `/updatefirstmate` skill owns that pass, and `bin/fm-secondmate-restart.sh` owns its staleness read, persist gate, busy deferral, replacement verification, and failure vocabulary.
