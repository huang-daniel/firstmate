...
A successful update evaluates stale live mates of both placements, including one whose home was already on the target commit, and restarts only mates with proven idle records, which today means a local claude mate, the one secondmate harness `bin/fm-spawn.sh` arms a busy record for; the `/updatefirstmate` skill owns that pass, and `bin/fm-secondmate-restart.sh` owns its staleness read, persist gate, busy deferral, replacement verification, and failure vocabulary.
