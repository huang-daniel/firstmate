...
| `fm-secondmate-restart.sh` | Skip current mates, persist open conversational work, then restart stale mates that are not provably busy and verify each replacement, or report the fallback outcome |
...
| `fm-validation-slot.sh`  | Read a repository's active validation runs across every clone and admit a new run under its ceiling |
...
| `fm-control.sh`          | Agent lifecycle control plane: allowlisted `interrupt`, `exit`, `stand-down`, and transactional `relaunch` verbs for an exact task id ([agent-control.md](agent-control.md)) |
