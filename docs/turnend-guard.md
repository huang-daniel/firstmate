...
A claude secondmate's semantic busy record (`bin/fm-busy-lib.sh`) is closed at `Stop` by this guard rather than by a hook of its own, because an exit 2 continues the same turn with no new prompt.
On exit 0 only, and only when the `FM_SECONDMATE_BUSY_*` launch binding is present, the home's marker names the same id, and this session owns the home lock, the guard applies one `idle` event through that binding's writer; any refusal leaves the record busy.
Claude processes an `asyncRewake` only after every synchronous `Stop` hook returns, so this idle always precedes the rewake's `UserPromptSubmit` busy.
`bin/fm-spawn.sh`'s secondmate arm owns the binding, and [`verification/supervision.md`](verification/supervision.md#claude-secondmate-busy-state-2026-09-23) records the live evidence.
