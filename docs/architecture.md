...
The cursor header in `bin/fm-classify-lib.sh` owns incremental read and line-boundary semantics.
...
A queued signal annotation prints every complete status line still unread at that cursor, holding a trailing line the writer has not yet terminated for the next drain as selected by `fm_wake_unread_events` in `bin/fm-wake-lib.sh`, while the fleet-wide UNREAD STATUS section prints `note:` lines and reserved-key pending-reply resolutions once even on an empty-queue drain because those verbs never enter the OPEN DECISIONS fold.
...
The answering contract and its refusal conditions are owned by the `bin/fm-send.sh` header; remote escalations reach the local ledger through parent-replies ingest, while the answer message crosses the transport.
...
Any direct or remaining historical annotation prints every complete status line unread at the presentation cursor instead of replaying only the latest line.
...
Because a silent success is indistinguishable from a broken transport, and reading it as one is how a single steer became three stacked copies, neither half of that ambiguity is left to the reader's memory: every inbox-plane send names the record it queued, and a local steer whose exact instruction is still queued and unhandled for that task collapses onto it instead of enqueueing a second copy, with `--again` reserved for a deliberate repeat.
Record identity for both that collapse and the remote leg's wider idempotent enqueue is the body's exact bytes plus its delivery class within one task, and every ambiguity resolves toward enqueueing or failing loudly rather than collapsing, because a duplicate is visible to the worker while a swallowed steer is not (`bin/fm-task-inbox-lib.sh` header).
