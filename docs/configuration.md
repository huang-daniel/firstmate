...
The same bootstrap run emits `SECONDMATE_LIVENESS:` only when a registered secondmate is skipped, its relaunch fails, or a lock collision prevents or refuses the relaunch; already-live and successfully relaunched secondmates are handled silently, and every outcome that leaves a home without a running mate also queues a durable check notification ([`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh) owns the line formats).
