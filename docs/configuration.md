...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, sanitized per-task timelines kept at cleanup as `data/<id>/timeline.tsv` ([`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh) owns their fields and sanitization), per-task ledgers of red checks a merge waived as `data/<id>/merge-waivers.tsv` ([`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh) owns their fields), and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
For Treehouse-backed tasks, each home draws worker copies only from its own pools, kept outside every home under `$HOME/.firstmate-treehouse/`; [`fm_treehouse_home_root`](../bin/fm-wake-lib.sh) owns the per-home root and why it exists.
...
The [Claude adapter reference](../.agents/skills/harness-adapters/references/harness/claude.md) records the verified shape of both launches and the confirmation dialogs each one can meet.
...
The same bootstrap run emits `SECONDMATE_LIVENESS:` only when a registered secondmate is skipped, its relaunch fails or cannot be verified, or a live or unreadable lock prevents the relaunch; already-live and verified replacement secondmates are handled silently, and every outcome that leaves a home without a running mate also queues a durable check notification ([`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh) owns the line formats).
...
FM_SECONDMATE_IDLE_SETTLE=120  # seconds after a second mate's persist answer during which bin/fm-secondmate-restart.sh re-reads a busy mate on its persist poll and restarts it on the first proven idle; a mate still busy when it closes is deferred, and 0 reads once (the script header owns the pass)
