...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, sanitized per-task timelines kept at cleanup as `data/<id>/timeline.tsv` ([`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh) owns their fields and sanitization), and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
