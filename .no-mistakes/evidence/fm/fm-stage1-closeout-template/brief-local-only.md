...
1. First action: create your branch: `git checkout -b fm/cl-local-only`

# Rules
1. Never push to any remote and never open a PR. Work only on your `fm/cl-local-only` branch; firstmate handles the merge into local `main`.
2. Stay inside this worktree; modify nothing outside it.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/tmp.e9EDASKsTD/state/cl-local-only.status'`
...
Firstmate steers you through durable message files in '/tmp/tmp.e9EDASKsTD/state/cl-local-only.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/tmp.e9EDASKsTD/state/cl-local-only.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/tmp.e9EDASKsTD/state/cl-local-only.inbox'/NNN.msg '/tmp/tmp.e9EDASKsTD/state/cl-local-only.inbox'/handled/`.
...
The task is complete only when committed on your branch `fm/cl-local-only`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if `main` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append `working [at=<epoch>]: implementation complete` to the status file as the implementation-complete event.
Then append `done [at=<epoch>]: ready in branch fm/cl-local-only` to the status file and stop.
