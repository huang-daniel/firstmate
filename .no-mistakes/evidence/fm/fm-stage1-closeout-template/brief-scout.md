...
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/tmp.e9EDASKsTD/state/cl-scout.status'`
...
Firstmate steers you through durable message files in '/tmp/tmp.e9EDASKsTD/state/cl-scout.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/tmp.e9EDASKsTD/state/cl-scout.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/tmp.e9EDASKsTD/state/cl-scout.inbox'/NNN.msg '/tmp/tmp.e9EDASKsTD/state/cl-scout.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Definition of done
Write your findings to `/tmp/tmp.e9EDASKsTD/data/cl-scout/report.md`.
...
Before reporting done, read and follow `/home/dev/.no-mistakes/worktrees/222d8f0053d8/01M31K61W2HVWQFPEFWGXNPVT5/.agents/skills/captain-hold-lifecycle/SKILL.md` and pass its shared completion gate for the report and any visual review.
