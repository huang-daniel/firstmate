...
config/secrets/  reference-only v2 credential manifests; values live separately in config/secret-values/; LOCAL, gitignored, not inherited. Use bin/fm-secret.sh list/names for inspection and reveal/export for explicit value reads; use export instead of sourcing files; see docs/configuration.md "Credential store"
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  board-lanes.tsv  durable persistent-lane records; firstmate-private, written only by bin/fm-board.sh, and kept after teardown so a standing charter is never offered for decomposition or import again; see board-orchestration
...
state/               runtime records and signals; gitignored
  ...
  <id>.herdr-presentation  quarantinable attempt, restart-binding, and flat-fallback journal for Herdr's optional visual projection; never task or endpoint authority; see docs/herdr-backend.md "Presentation spaces"
  ...
  .session-revision  the revision and instruction surface the lock-holding session started on, which a parent reads to tell a stale second mate from a current one; written only by bin/fm-secondmate-health.sh record
...
Every network check a session start owes - GitHub auth, dead-secondmate relaunch, secondmate convergence, pending handoff delivery, project clone refresh, and configured project board reconciliation - runs off the digest's blocking path in a bounded worker owned by `bin/fm-startup-network.sh` and is reported in the digest's own `NETWORK CHECKS` section.
...
Never merge a red PR under either setting unless a current explicit captain instruction names the set of GitHub checks waived through `fm-pr-merge.sh --allow-red`; that attended-only waiver still requires every other check green.
...
The primary admits every pipeline start on a project with a validation ceiling through `bin/fm-validation-slot.sh` before the worker is triggered, whichever home owns the crew, with `FM_HOME` set to that home.
...
- `project-management` - load before adding, creating, removing, or initializing a project.
  Cloning or registering a project is add intake and uses the same trigger.
- `board-orchestration` - load on a session-start or heartbeat cycle when `config/boards` configures a project board, before judging whether a filed card is one task, a programme, or a persistent lane, before importing, promoting, decomposing, declaring or reversing a lane, or placing a board item into or onto the backlog, before reflecting a dispatch, PR, blocker, or merge onto a board, before dispatching a task whose issue is a campaign child, and whenever a card's status or classification disagrees with firstmate's own records or a cycle reports a card unclassified.
