...
# `#{pane_current_path}` probe from FM_FAKE_PANE_PATH, and leases that same path
# from its fake treehouse, so this same stub works for a crew/scout
# (non-secondmate) spawn's leased-worktree wait loop.
...
test_spawn_secondmate_harness_model_token() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false},\"hooks\":{" \
    "model-token: launch did not carry the secondmate settings with its busy hooks"
...
test_spawn_secondmate_harness_model_and_effort_tokens() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false},\"hooks\":{" \
    "model-effort-tokens: launch did not carry the secondmate settings with its busy hooks"
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  printf '%s\n' 'Usage: treehouse get [--lease] [--root <dir>]'
fi
exit 0
SH
...
test_spawn_secondmate_claude_permission_mode_auto() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --permission-mode auto --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false},\"hooks\":{" \
    "permmode: launch did not carry the secondmate settings with its busy hooks"
  assert_contains "$launch" "}]}]}}' --model 'opus'" \
